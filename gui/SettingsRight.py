...
class SettingsRight(tk.Frame):
	...
	object that creates this class, and 'thermostat' which is the thermostat object created at the beginning of execution.'''
...
# --------------------- Init Function  ---------------------
	...
	def __init__(self, parent, thermostat):
		...
		self.pwr_option = tk.Label(self, text='Thermostat Power', font=('calibri', 14), bg='#525252', fg='white')
		...
		#reboot option
		self.reboot_option = tk.Label(self, text='Reboot Thermostat', font=('calibri', 14), bg='#525252', fg='white')
		self.reboot_option.grid(row=1, column=0, sticky='w', padx=(10, 0))

		self.reboot_btn = tk.Label(self,text='Reboot', font=('calibri', 14), bg='#cdcdcd', fg='#353535')
		...
		self.temp_range = tk.Label(self, text='Temp Range', font=('calibri', 14), bg='#525252', fg='white')
		self.temp_range.grid(row=2, column=0, sticky='w', padx=(10, 0))
		#call function that creates slider
		self._update_slider()

		#fahrenheit or celcius setting
		self.temp_option = tk.Label(self, text='Temp Reading', font=('calibri', 14), bg='#525252', fg='white')
		...
		self.mode_option = tk.Label(self, text='Thermostat Mode', font=('calibri', 14), bg='#525252', fg='white')
...
# --------------------- Helper Functions  ---------------------
	...
	def _toggle_power(self):
		...
		self.off_btn.config(bg='#cdcdcd' if self._therm.is_on else '#353535', 
						  	fg='#353535' if self._therm.is_on else '#cdcdcd')
		self.on_btn.config(bg='#353535' if self._therm.is_on else '#cdcdcd',
						   fg='#cdcdcd' if self._therm.is_on else '#353535')
	...
	def _toggle_temp(self):
		...
		self.c_btn.config(bg='#cdcdcd' if self._therm.degree == 'F' else '#353535', 
						  fg='#353535' if self._therm.degree == 'F' else '#cdcdcd')
		self.f_btn.config(bg='#353535' if self._therm.degree == 'F' else '#cdcdcd',
						  fg='#cdcdcd' if self._therm.degree == 'F' else '#353535')

		self._therm.toggle_degree()
		self._parent.update_curr_temp()
		self._update_slider()

	def _toggle_mode(self):
		'''Toggle between adaptive and manual mode'''
		self.manual_btn.config(bg='#cdcdcd' if self._therm.system == 'Adaptive' else '#353535',
							   fg='#353535' if self._therm.system == 'Adaptive' else '#cdcdcd')
		self.adaptive_btn.config(bg='#353535' if self._therm.system == 'Adaptive' else '#cdcdcd',
								 fg='#cdcdcd' if self._therm.system == 'Adaptive' else '#353535')
