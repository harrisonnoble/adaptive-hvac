...
class SettingsSensors(tk.Frame):
	'''SettingsSensors handles displaying the sensor outputs in the left side of the settings tab. Arguements are: 'parent' 
	which is the frame object that creates this class, and 'thermostat' which is the thermostat object created at the 
...
# --------------------- Init Function  ---------------------
	...
	def __init__(self, parent, thermostat):
		...
		self.temp_sensor_lbl = tk.Label(self, text='Temperature Sensor', font=('calibri', 14), bg='#525252', fg='white')
		self.temp_sensor_lbl.grid(row=0, column=0, sticky='w', padx=(10, 0))
		
		self.temp_sensor = tk.Label(self, text=str(self._therm.curr_temp) + '°', font=('calibri', 14), bg='#525252', fg='white')
		self.temp_sensor.grid(row=0, column=1, sticky='e', padx=(0, 10))

		#create and display motion sensor reading
		self.motion_sensor_lbl = tk.Label(self, text='Motion Sensor', font=('calibri', 14), bg='#525252', fg='white')
		self.motion_sensor_lbl.grid(row=1, column=0, sticky='w', padx=(10, 0))

		self.motion_sensor = tk.Label(self, font=('calibri', 14), bg='#525252', fg='white',
		...
		self.audio_sensor_lbl = tk.Label(self, text='Audio Sensor', font=('calibri', 14), bg='#525252', fg='white')
		self.audio_sensor_lbl.grid(row=2, column=0, sticky='w', padx=(10, 0))

		self.audio_sensor = tk.Label(self, font=('calibri', 14), bg='#525252', fg='white',
		...
		self.camera_lbl = tk.Label(self, text='People Detected', font=('calibri', 14), bg='#525252', fg='white')
		self.camera_lbl.grid(row=3, column=0, sticky='w', padx=(10, 0))

		self.camera = tk.Label(self, text=str(self._therm.num_people), font=('calibri', 14), bg='#525252', fg='white')
		self.camera.grid(row=3, column=1, sticky='e', padx=(0, 10))

		#create and display room size reading
		self.room_size_lbl = tk.Label(self, text='Room Size Sensor', font=('calibri', 14), bg='#525252', fg='white')
...
# --------------------- Helper Functions  ---------------------
	...
	def _update_sensors(self):
		...
		self._stopper_sensors = self.after(200, self._update_sensors)
	...
	def stopper(self):
		...
		if self._stopper_sensors:
			self.after_cancel(self._stopper_sensors)
			self._stopper_sensors = None
